...
def weight_matrix(W,sigma2=0.1,epsilon=0.5):
    ...
    '''
    # sigma2 = np.max(W)*np.max(W)/np.log(2) # sigma2=5.20 for PEMSBAY
...
def main():
    ...
    ks, kt, bs, T, n, p = 3, 3, [[1, 32, 64], [64, 32, 128]], 12, 207, 0
    A = pd.read_csv('../data/stgcn-data/W_metrla.csv').values
    W = weight_matrix(A)
    L = scaled_laplacian(W)
    Lk = cheb_poly(L, ks)
    Lk = torch.Tensor(Lk.astype(np.float32)).to(device)
    model = STGCN(ks, kt, bs, T, n, Lk, p).to(device)
    summary(model, (1, 12, 207), device="cuda:{}".format(GPU))
